...
namespace Logger {
  ...
  class LogWriter {
    public:
      ...
      void debug(const char* filename, int line, const String &message) {
        log(Priority::DEBUG, filename, line, message);
      }

      /** Prints a message with variable arguments */
      void log(int priority, const char* filename, int line, const char *format, ...);

      /** Prints a message */
      void log(int priority, const char* filename, int line, const String &message) {
        log_string(priority, filename, line, message.c_str());
      }

      /** Prints a message */
      void log(int priority, const String &message) {
        log_string(priority, "", 0, message.c_str());
      }

    private:
      /** Appends a string message to the log */
      void log_string(int priority, const char* filename, int line, const char *message);

      /** Appends a string message with variable arguments to the log */
      void log_varargs(int priority, const char* filename, int line, const char *format, va_list ap);
...
#define HT_LOG(priority, msg) do { \
  if (Logger::get()->is_enabled(priority)) { \
      Logger::get()->log(priority, __FILE__, __LINE__, msg); \
  } \
} while (0)

#define HT_LOGF(priority, fmt, ...) do { \
  if (Logger::get()->is_enabled(priority)) { \
      Logger::get()->log(priority, __FILE__, __LINE__, Hypertable::format( \
          fmt, __VA_ARGS__));  \
  } \
} while (0)
...
#define HT_LOG_ENTER do { \
  if (Logger::get()->is_enabled(Logger::Priority::DEBUG)) {\
      Logger::get()->debug(__FILE__, __LINE__, "%s() ENTER", HT_FUNC); \
  } \
} while(0)

#define HT_LOG_EXIT do { \
  if (Logger::get()->is_enabled(Logger::Priority::DEBUG)) { \
      Logger::get()->debug(__FILE__, __LINE__, "%s() EXIT", HT_FUNC); \
  } \
} while(0)
