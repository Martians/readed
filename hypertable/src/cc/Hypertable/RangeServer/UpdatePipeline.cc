...
void
update_table_error(int type, UpdateRecTable *table_update)
{
	update_dec(type, table_update->requests.size());
	update_dec(type + 1, table_update->total_count);
	update_dec(type + 2, table_update->total_buffer_size);

	if (type != ST_respons) {
		update_inc(ST_respons, table_update->requests.size());
		update_inc(ST_respons_cell, table_update->total_count);
		update_inc(ST_respons_size, table_update->total_buffer_size);
	}
}
...
#if TESTING_SUPPORT
  if (Global::user_log == m_log) {
  		set_thread("_req");
  	}

  	if (Global::system_log == m_log) {
  		set_thread("s_req");
  	}

  	if (Global::metadata_log == m_log) {
  		set_thread("m_req");
  	}

  	if (Global::root_log == m_log) {
  		set_thread("r_req");
  	}
#endif
  while (true) {
    ...
    for (UpdateRecTable *table_update : uc->updates) {
      ...
      if (!table_update->id.is_system() && m_context->server_state->readonly()) {
        ...
        update_table_error(ST_request, table_update);
        continue;
      }
      ...
      time_bomb("get table info too long");
      ...
      for (UpdateRequest *request : table_update->requests) {
        ...
        while (mod < mod_end) {
          ...
          time_bomb("get find range in table too long");
          update_inc(ST_table_lock, time_last());
          ...
          time_bomb("get range lock for range error, too long, name " << rulist->range->get_name());
          update_inc(ST_range_lock, time_last());
          ...
          if (!rulist->range_blocked) {
            ...
            time_bomb("get range lock: increase update counter, too long, name " << rulist->range->get_name());
            update_inc(ST_range_barrier, time_last());
          ...
          time_bomb("get range meta log too long, name " << rulist->range->get_name());
          update_inc(ST_range_metalog, time_last());
          ...
          /** Fetch range transfer information **/
          {
            ...
            time_bomb("get range transfer info too long, name " << rulist->range->get_name());
            update_inc(ST_range_lock, time_last());
          }
          ...
          if (request->error == Error::OK) {
             ...
             */
        	time_debug_using("range error, cancel reqeust: " << Error::get_text(request->error));
        	update_inc(ST_range_error);
        ...
        for (auto it = request->send_back_vector.begin();
        	it != request->send_back_vector.end(); it ++)
        {
        	cancel_count += (*it).count;
        	cancel_size  += (*it).len;

        	update_inc(ST_cancel);
        	update_inc(ST_cancel_cell, (*it).count);
        	update_inc(ST_cancel_size, (*it).len);
        }
      }

      if (cancel_count > 0) {
    	  time_debug_using("cancel request, for range not belong to us, count "
    			  << cancel_count << ", size " << string_size(cancel_size));
      }
      ...
      time_bomb("parse table info too long, name " << rulist->range->get_name());

      //update_inc(ST_process, uc->total_updates);
      //update_inc(ST_process_cells, uc->total_added);
      update_next(ST_commit, table_update->requests.size());
      update_next(ST_commit_cell, table_update->total_count);
      update_next(ST_commit_size, table_update->total_buffer_size);
    }
...
void UpdatePipeline::commit() {
  ...
  while (true) {
    ...
    if (uc->root_buf.ptr > uc->root_buf.mark) {
    	//update_inc(ST_commit, uc->total_updates);
    	//update_inc(ST_commit_cell, uc->total_added);
      ...
      time_debug_using("commit: write root log complete");
    }
    for (UpdateRecTable *table_update : uc->updates) {
    	 time_reset();
      ...
      for (auto iter = table_update->range_map.begin(); iter != table_update->range_map.end(); ++iter) {
        if ((*iter).second->transfer_buf.ptr > (*iter).second->transfer_buf.mark) {
          ...
          if ((error = (*iter).second->transfer_log->write(ClusterId::get(), (*iter).second->transfer_buf,
														   ...
														   Filesystem::Flags::NONE)) != Error::OK) {
          ...
          uint64_t len = (*iter).second->transfer_buf.ptr - (*iter).second->transfer_buf.mark;
          transfer_len += len;

          update_inc(ST_transfer);
          update_inc(ST_transfer_size, len);
          update_inc(ST_transfer_time, time_last());
        }
      }
      if (transfer_len) {
    	  time_trace_using("commit: write transfer log, " << string_size(transfer_len));
      }

      if (table_update->error != Error::OK) {
    	  update_table_error(ST_commit, table_update);
    	  time_debug_using("commit: table error");
        continue;
      }
      ...
      update_next(ST_respons, table_update->requests.size());
      update_next(ST_respons_cell, table_update->total_count);
      update_next(ST_respons_size, table_update->total_buffer_size);

      update_inc(ST_commit_done, table_update->requests.size());
	  update_inc(ST_commit_done_cell, table_update->total_count);
	  update_inc(ST_commit_done_size, table_update->total_buffer_size);
    }
    if (coalesce_amount == 0 && committed_transfer_data == 0) {
        time_trace_using("commit do nothing");
        log_needs_syncing = false;

    } else {
    	/*
		time_trace_using("commit: rqst " << m_qualify_queue.size()
			<< ", wait " << m_commit_queue_count
			<< ", coal " << coalesce_queue.size()
			<< ", resp " << m_respons_queue_count
			<< ", current length for sync " << StringSize(coalesce_amount));
		*/
		try_next = m_commit_queue_count == 0 ? false : true;
    }
    bool do_sync = false;
...
void UpdatePipeline::add_and_respond() {
  ...
  while (true) {
    ...
    for (UpdateRecTable *table_update : uc->updates) {
      ...
      for (auto iter = table_update->range_map.begin(); iter != table_update->range_map.end(); ++iter) {
        ...
        for (UpdateRecRange &update : (*iter).second->updates) {
          ...
          time_bomb("resp lock too long, name " << rangep->get_name());
          update_inc(ST_range_lock, time_last());
		  ...
		  time_bomb("add cell and query cache too long, name " << rangep->get_name()
			  << " size " << string_size(update.len) << ", tree cell count " << string_count(rangep->DumpCacheCount()));

          if (0 && m_query_cache && current_row) {
            if (invalidate)
              columns.clear();
            m_query_cache->invalidate(table_update->id.id, current_row, columns);
          }
    ...
    time_bomb("release lock too long");
	...
	Response::Callback::Update cb(m_context->comm, request->event);
        ...
        if (table_update->error != Error::OK) {
        	update_table_error(ST_respons, table_update);
        ...
        update_dec(ST_respons, table_update->requests.size());
		update_dec(ST_respons_cell, table_update->total_count);
		update_dec(ST_respons_size, table_update->total_buffer_size);
      }
    }
