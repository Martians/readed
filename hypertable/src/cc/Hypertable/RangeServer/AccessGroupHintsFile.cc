...
void AccessGroupHintsFile::write(String location) {
 ...
 try_again:
  ...
  try {
    if (!first_try) {
      if (fd != -1)
        Global::meta_dfs->close(fd);
      if (!Global::meta_dfs->exists(parent_dir))
        Global::meta_dfs->mkdirs(parent_dir);
    }
    fd = Global::meta_dfs->create(parent_dir + "/hints",
                             Filesystem::OPEN_FLAG_OVERWRITE, -1, -1, -1);
    StaticBuffer sbuf(contents.length());
    memcpy(sbuf.base, contents.c_str(), contents.length());
    Global::meta_dfs->append(fd, sbuf);
    Global::meta_dfs->close(fd);
  }
  catch (Exception &e) {
  ...
  time_debug_using("write range hint file");
...
void AccessGroupHintsFile::read() {
  ...
  try {
    ...
    int64_t length = Global::meta_dfs->length(filename);
    const char *base;

    dbuf.grow(length+1);

    fd = Global::meta_dfs->open(filename, Filesystem::OPEN_FLAG_VERIFY_CHECKSUM);
    size_t nread = Global::meta_dfs->read(fd, dbuf.base, length);
    Global::meta_dfs->close(fd);
    dbuf.base[nread] = 0;
...
bool AccessGroupHintsFile::exists() {
  ...
  if (!Global::meta_dfs->exists(path))
    return false;

  path.append("/hints");

  return Global::meta_dfs->exists(path);
