...
#if TESTING_SUPPORT
std::string
TableInfo::DumpStat(DumpInfo* stat)
{
	std::vector<RangePtr> vec;
	{
		lock_guard<mutex> lock(m_mutex);
		if (m_identifier.is_metadata() ||
			m_identifier.is_system()) return "";

		for (auto iter = m_active_set.begin();
			iter != m_active_set.end(); iter++)
		{
			vec.push_back(iter->range);
		}
	}

	char temp[64* 1024];
	char* data = temp;
	data += snprintf(data, sizeof(temp), "\n  table: %s, type: %s, range: %d\n",
			m_identifier.id, m_identifier.is_metadata() ? "meta" :
			(m_identifier.is_system() ? "system" : "user"), (int)m_active_set.size());

	int index = 0;
	for (auto iter = vec.begin(); iter != vec.end(); iter++) {
		RangePtr range = *iter;
		data += snprintf(data, sizeof(temp), "\t <%-2d %s\n", index++,
			range->DumpStat(stat).c_str());
	}
	return temp;
}
#endif
