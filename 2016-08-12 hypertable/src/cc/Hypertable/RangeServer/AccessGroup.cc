...
#undef	TIME_PREFIX
#define TIME_PREFIX "____________access "
#endif
...
#if TESTING_SUPPORT
std::string
AccessGroup::DumpStat(DumpInfo* stat)
{
	DumpInfo curr;
	space_usage(&curr.cache, &curr.disk);
	curr.file_count = m_stores.size();

	CellStore::IndexMemoryStats mem_stat;
	for (size_t i=0; i<m_stores.size(); i++) {
		m_stores[i].cs->get_index_memory_stats(&mem_stat);
		curr.index += mem_stat.block_index_memory;
		curr.index += mem_stat.bloom_filter_memory;
	}

	CellCache::Statistics cache_stat;
	m_cell_cache_manager->get_cache_statistics(cache_stat);

	char data[1024];
	snprintf(data, sizeof(data), "disk: %8s, mem: %8s, file: %lld, index: %4s",
			string_size(curr.disk).c_str(), string_size(curr.cache).c_str(),
			(UINT64)curr.file_count, string_size(curr.index).c_str());
	...
	if (stat) {
		...
		stat->cache_count += cache_stat.size;
	}
	return data;
}

uint64_t
AccessGroup::DumpCacheCount(bool total)
{
	return m_cell_cache_manager->DumpCacheCount(total);
}
#endif
...
void AccessGroup::run_compaction(int maintenance_flags, Hints *hints) {
  ...
  int compact_type = 0;
  time_bomb(TIME_PREFIX << "compact, determinate type");
  ...
  try {
...

    {
      ...
      if (m_in_memory) {
        ...
        if (m_end_merge) {
          ...
          m_cell_cache_manager->add_immutable_scanner(mscanner.get(), scan_ctx.get());
          if (m_cell_cache_manager->immutable_cache()) {
			cache += m_cell_cache_manager->immutable_cache()->logical_size();
		  }
        }
        else
          max_num_entries = 0;
        for (size_t i=merge_offset; i<merge_offset+merge_length; i++) {
          HT_ASSERT(m_stores[i].cs);
          mscanner->add_scanner(m_stores[i].cs->create_scanner(scan_ctx.get()));
          read += m_stores[i].key_bytes + m_stores[i].value_bytes;
        ...
        file_count = merge_length;

        compact_type = ST_compact_merge;
      }
      else if (major) {
        mscanner = make_shared<MergeScannerAccessGroup>(m_table_name, scan_ctx.get(), 
                                                        MergeScannerAccessGroup::IS_COMPACTION |
        ...
        file_count = m_stores.size();

        compact_type = ST_compact_major;
      }
      else {
        ...
        compact_type = ST_compact_minor;
      }
    }

    range_inc(compact_type);
    range_inc(ST_try_rw, read);
    range_inc(ST_try_mem, cache);
    ...
    if (mscanner) {
      while (mscanner->get(key, value)) {
        ...
        if (count++ == 1000) {
        	range_inc(ST_done_rw, mscanner->get_input_cells() - total);
        	total = mscanner->get_input_cells();
        	count = 0;
        }
      }
    ...
    range_dec(ST_try_rw, read);
    range_dec(ST_try_mem, cache);
    range_inc(ST_done_mem, cache);

    time_debug_mute_warn(TIME_PREFIX << "compact, " << (major ? "major" : (merging ? "merge" : "minor"))
    	<< " scanned, file: " << string_size(read) << " cache: " << string_size(cache)
		<< ", [" << m_stores.size() << " -> " << m_stores.size() - file_count + 1 << "] read: "
		<< string_speed(read, time_last()) << " write: " << string_speed(read + cache, time_last()));
  ...
  if (compact_type != 0) {
	  range_dec(compact_type);
	  range_inc(ST_compact_done);
	  time_debug_mute_warn(TIME_PREFIX << "complete compact");
  }
}
