...
#if TESTING_SUPPORT
#	include <LocalClient/DumpStat.cpp>
...
Apps::RangeServer::RangeServer(PropertiesPtr &props, ConnectionManagerPtr &conn_mgr,
  ...
  Global::meta_dfs = std::make_shared<FsBroker::Lib::LocalClient>(props, 5, false);
  if (0 && props->get_bool("FsBroker.Local.CellStore")) {
	  HT_INFO("using local api for cellstore");
	  Global::dfs = std::make_shared<FsBroker::Lib::LocalClient>(props, 20, false);
  ...
  if (1 || props->get_bool("FsBroker.Local.CommitLog")) {
	  HT_INFO("using local api for commit log");
	  Global::log_dfs = std::make_shared<FsBroker::Lib::LocalClient>(props, 5, true);
	  ...
	  if (cfg.has("CommitLog.DfsBroker.Host")) {
		...
		FsBroker::Lib::ClientPtr dfsclient = std::make_shared<FsBroker::Lib::Client>(conn_mgr, addr, dfs_timeout);
  ...
  Global::transfer_dfs = Global::meta_dfs;
...
void Apps::RangeServer::local_recover() {
  ...
  try {
    ...
    if (!entities.empty()) {
          ...
          if (dynamic_pointer_cast<MetaLog::EntityTask>(entity))
            ...
            if ((range_entity->get_state() & RangeState::PHANTOM) != 0) {
              ...
              if (strstr(transfer_log.c_str(), "phantom") != 0) {
                try {
                	Global::transfer_dfs->rmdir(transfer_log);
                }
                catch (Exception &e) {
      ...
      if (!replay_map.empty()) {
        root_log_reader = make_shared<CommitLogReader>(Global::log_dfs, Global::transfer_dfs,
                                              Global::log_dir + "/root");
      ...
      if (root_log_reader)
        Global::root_log = make_shared<CommitLog>(Global::log_dfs, Global::transfer_dfs, Global::log_dir
      ...
      if (!replay_map.empty()) {
        metadata_log_reader =
          make_shared<CommitLogReader>(Global::log_dfs, Global::transfer_dfs, Global::log_dir + "/metadata");
      ...
      if (root_log_reader || metadata_log_reader) {
        Global::metadata_log = make_shared<CommitLog>(Global::log_dfs, Global::transfer_dfs,
      ...
      if (!replay_map.empty()) {
        system_log_reader =
          make_shared<CommitLogReader>(Global::log_dfs, Global::transfer_dfs, Global::log_dir + "/system");
      ...
      if (system_log_reader) {
        Global::system_log = make_shared<CommitLog>(Global::log_dfs, Global::transfer_dfs,
      ...
      if (!replay_map.empty()) {
        user_log_reader = make_shared<CommitLogReader>(Global::log_dfs, Global::transfer_dfs,
                                              Global::log_dir + "/user");
      ...
      Global::user_log = make_shared<CommitLog>(Global::log_dfs, Global::transfer_dfs, Global::log_dir
      ...
      if (root_log_reader || metadata_log_reader) {
        Global::metadata_log = make_shared<CommitLog>(Global::log_dfs, Global::transfer_dfs, Global::log_dir
      ...
      if (system_log_reader) {
        Global::system_log = make_shared<CommitLog>(Global::log_dfs, Global::transfer_dfs, Global::log_dir
...
Apps::RangeServer::load_range(ResponseCallback *cb, const TableIdentifier &table,
  ...
  try {
    ...
    time_debug_using("try load range, " << range_spec << ", compact " << needs_compaction);
...
Apps::RangeServer::update(Response::Callback::Update *cb, uint64_t cluster_id,
  ...
  update_inc(ST_request);
  update_inc(ST_request_cell, count);
  update_inc(ST_request_size, buffer.size);
...
void Apps::RangeServer::replay_fragments(ResponseCallback *cb, int64_t op_id,
  ...
  try {
    log_reader = make_shared<CommitLogReader>(Global::log_dfs, Global::transfer_dfs, log_dir, fragments);
    StringSet receivers;
        ...
        if (rr.table.is_metadata()) {
          if (rr.is_root()) {
            if (!Global::root_log) {
              Global::log_dfs->mkdirs(Global::log_dir + "/root");
              if (!Global::root_log)
                Global::root_log = make_shared<CommitLog>(Global::log_dfs, Global::transfer_dfs,
...
void Apps::RangeServer::do_maintenance() {
  ...
  try {
	set_thread("schedule", ThreadInfo::TT_pool);
