...
void TableInfoMap::promote_staged_range(const TableIdentifier &table, RangePtr &range, const char *transfer_log) {
  ...
  if (transfer_log && *transfer_log) {
    CommitLogReaderPtr commit_log_reader =
      make_shared<CommitLogReader>(Global::log_dfs, Global::transfer_dfs, transfer_log);
...
#if TESTING_SUPPORT
std::string
TableInfoMap::DumpStat(DumpInfo* stat)
{
	int32_t count = 0;
	std::vector<TableInfoPtr> vec;
	{
		lock_guard<mutex> lock(m_mutex);
		for (InfoMap::iterator iter = m_map.begin();
			iter != m_map.end(); iter++)
		{
			count += (*iter).second->get_range_count();
	...
	char data[64 * 1024];
	snprintf(data, sizeof(data), "table count %d,  range count: %d",
			(int) m_map.size(), count);

	for (auto iter = vec.begin(); iter != vec.end(); iter++) {
		strcat(data, (*iter)->DumpStat(stat).c_str());
	}
	return data;
}
#endif
