...
CellStoreV7::create(const char *fname, size_t max_entries,
  ...
  //s_file.open(m_filename, FileBase::op_create | FileBase::op_read | FileBase::op_write | FileBase::op_trunc);
...
void CellStoreV7::add(const Key &key, const ByteString value) {
  ...
  if (m_buffer.fill() > (size_t)m_uncompressed_blocksize) {
    ...
    //s_file.write(send_buf.base, send_buf.size);
...
void CellStoreV7::finalize(TableIdentifier *table_identifier) {
  ...
  m_filesys->close(m_fd);
  //int64_t length = s_file.file_len();
//  if (0 && length != m_filesys->length(m_filename)) {
//	  time_debug_using("origin " << length << ", wrong " << m_filesys->length(m_filename));
//	  exit(0);
//  }
