...
uint64_t
CellCacheManager::DumpCacheCount(bool total)
{
	uint64_t count = m_active_cache->DumpCacheCount();
	if (total && m_immutable_cache)
		count += m_immutable_cache->DumpCacheCount();
	return count;
}
