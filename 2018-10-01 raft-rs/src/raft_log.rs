...
impl<T: Storage> RaftLog<T> {
    ...
    pub fn maybe_append(
        ...
        if self.match_term(idx, term) {
            ...
            if conflict_idx == 0 {
                info!("  ----> follower {} recv: {}, no need append", self.tag, ents.len());
                ...
                let offset = idx + 1;
                info!("  ----> follower {} recv: index {}, count {}", self.tag, idx + 1, ents.len() as u64 - (conflict_idx - offset));
